...
[filename,path] = uigetfile('C:\Users\Janis\Documents\Munoz Lab\Data\indy\','multiselect','on');
...
file = cell2struct([filename,actualtask,depth,unit,isolation,rew1,rew2],{'name','task','depth','unit','isolation','rew1','rew2'},2);
multi4v3Index = [];
...
if strcmpi(task,'prior04')
    ...
    colors = {'m-','m:','g-','g:','m--','g--','m-.','g-.','m*','m.','g*','g.','mo','go','mp','gp'};
...
for x = 1:size(filename,1) % for each file
    if file(x).depth < 1000 && file(x).depth >= 0
        [convspks, lfp] = neural4(file(x),useOrig);
        if ischar(convspks)
            display(convspks)
            useless = useless + 1;
            continue
...
%             eval(['SCs' num2str(y-1) '(SCsN, :)=convspks_norm(y,:);']);
%             eval(['lfpS' num2str(y-1) '(SCsN, :)=lfp(y,:);']);
            eval(['lfpS' num2str(y-1) '= [lfpS' num2str(y-1) '; lfp(y,:)];']);
        end
    elseif file(x).depth >=1000
        [convspks, lfp] = neural4(file(x),useOrig);
        if ischar(convspks)
            useless = useless + 1;
            continue
...
%             eval(['SCi' num2str(y-1) '(SCiN, :)=convspks_norm(y,:);']);
%             eval(['lfpI' num2str(y-1) '(SCiN, :)=lfp(y,:);']);
            eval(['lfpI' num2str(y-1) '= [lfpI' num2str(y-1) '; lfp(y,:)];']);
        end
    end
    [pupil] = behaviorPrior2(file(x), useOrig);
    if ischar(pupil)
%         display(pupil)
%         file(x).name
        continue
...
if SCsN ~=0 || SCiN ~=0
    ...
    for x = 1:size(file(1).colors,2)/2
        eval(['avg_spks=nanmean(SCs' num2str(x-1) ',1);']);
        hSpk(x) = plot(tt,avg_spks(1,:),file(1).colors{x});
    end
    
    ymax=ceil(max(max(avg_spks,[],2)));
    ymin=floor(min(min(avg_spks,[],2)));
    ...
    tt=xmin-nstd:xmax+nstd-1;
    for x = 1:size(file(1).colors,2)/2
        eval(['avg_spks=nanmean(SCi' num2str(x-1) ',1);']);
        hSpk(x) = plot(tt,avg_spks(1,:),file(1).colors{x});
    end
    ...
    for x = 1:size(col,2)/2
        eval(['avg_spks=nanmean(lfpS' num2str(x-1) ',1);']);
        hSpk(x) = plot(tt,avg_spks(1,:),col{x});
    end
...
%     axis([0 200 ymin ymax])
    ...
    tt=xmin:xmax-1;
    for x = 1:size(col,2)/2
        eval(['avg_spks=nanmean(lfpI' num2str(x-1) ',1);']);
        hSpk(x) = plot(tt,avg_spks(1,:),col{x});
    end
...
%     axis([0 200 ymin ymax])
    ...
    for y = 1:size(file(1).condNames,1)-1 % for each condition
        figure
        hold on
